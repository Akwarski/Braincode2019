...
def calc_dist_to_km(x1,y1,x2,y2):
    return np.sqrt(((111.320 * np.cos(y2))*x2-(111.320 * np.cos(y1))*x1)^2 + ((110.574)*y2-(110.574)*y1)^2)

def find_closest(dlug, szer):
    indexes = []
    paczki = Paczkomat.objects.all()
    for paczka in paczki:
        if calc_dist_to_km(dlug,szer,paczka.dlugosc,paczka.szerokosc) <= 10:
